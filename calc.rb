...
require 'optparse'
require 'logger'
require_relative 'linear_regression_calculator'

$logger = Logger.new(STDOUT)
$logger.level = Logger::INFO

def usage
  'Usage: lin_reg_calc.rb --train FILE --input FILE '\
  '[--rate FLOAT] [--delimiter CHAR]'
end
...
def parse_options
  ...
  optparse = OptionParser.new do |opts|
    ...
    opts.on('-i', '--input FILE', 'Input file with required '\
            'predictions') do |file|
    ...
    options[:delimiter] = ','
    opts.on('-l', '--delimiter CHAR', 'Character delimiting the data '\
            '(default is \',\')') do |char|
...
def main
  ...
  if options[:training_file] && options[:input_file]
    ...
    $logger.info(sprintf('Total time=%ds', end_time - start_time))
